...
Automated launch file for joint trajectory tracking with MIT controller.

This test uses the OpenARM 7-DOF robot with zordi_joint_mit_controller
for trajectory tracking with gravity compensation.
...
def generate_launch_description():
    """Generate automated launch description for joint trajectory testing."""
    ...
    send_test_trajectory = ExecuteProcess(
        cmd=[
            ...
            "action",
            "send_goal",
            "/zordi_joint_mit_controller/follow_joint_trajectory",
    ...
    # Verification step: check final joint positions
    verify_result = ExecuteProcess(
        cmd=[
            "python3",
            str(verify_script),
            "--test-name",
            "Joint Trajectory (MIT Controller)",
            "--expected",
            expected_home,
            "--tolerance",
            "0.1",
    ...
    return LaunchDescription([
        ...
        load_mit_controller,
        # Chain: MIT loads -> reset -> unpause -> trajectory -> verify -> shutdown
        RegisterEventHandler(
